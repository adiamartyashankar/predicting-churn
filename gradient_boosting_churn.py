...
selected_features = correlations.index[1:21].tolist()  # top 20 features
print('Top features used for gradient boosting:')
...
# 6. Scale features (optional for boosting, but keeps pipeline consistent)
...
gs = GridSearchCV(gb, param_grid, cv=3, scoring='accuracy', n_jobs=-1, verbose=1)
gs.fit(X_train_scaled, y_train)

best_gb = gs.best_estimator_
print('Best Gradient Boosting Params:', gs.best_params_)
...
def save_metrics_to_file(iteration, accuracy, report, matrix, params, filename="gb_results.txt"):
